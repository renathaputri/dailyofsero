/**
 * Email abstraction service for GrowthWithSero
...
export async function sendEmail({ to, subject, html, text }: SendEmailParams): Promise<{ success: boolean; messageId?: string }> {
  ...
  const from = process.env.EMAIL_FROM || "GrowthWithSero <noreply@growthwithsero.com>";
...
export async function sendVerificationEmail(to: string, token: string, appUrl: string) {
  ...
  return sendEmail({
    to,
    subject: "✨ Verifikasi Akun Kamu di GrowthWithSero",
    html: `
      <div style="font-family: 'Montserrat', sans-serif; max-width: 600px; margin: auto; padding: 24px; background: #F8FAFC; border-radius: 16px;">
        ...
        <p style="color: #334155; font-size: 16px; line-height: 1.6;">
          Selamat datang di <strong>GrowthWithSero</strong>! Satu langkah lagi untuk mulai perjalanan healing dan journaling kamu.
        </p>
        ...
        <p style="color: #94A3B8; font-size: 12px; margin-top: 24px;">
          Tautan ini berlaku selama 24 jam. Jika kamu tidak merasa mendaftar di GrowthWithSero, abaikan email ini ya.
        </p>
      </div>
    `,
  });
...
export async function sendPasswordResetEmail(to: string, token: string, appUrl: string) {
  ...
  return sendEmail({
    to,
    subject: "🔑 Permintaan Reset Password — GrowthWithSero",
    html: `
      <div style="font-family: 'Montserrat', sans-serif; max-width: 600px; margin: auto; padding: 24px; background: #F8FAFC; border-radius: 16px;">
        <h2 style="color: #9333EA;">Reset Password Akun 🔒</h2>
        <p style="color: #334155; font-size: 16px; line-height: 1.6;">
          Kami menerima permintaan untuk mengatur ulang kata sandi akunmu di <strong>GrowthWithSero</strong>.
        </p>
        <div style="text-align: center; margin: 32px 0;">
...
export async function sendTakedownEmail(to: string, karyaTitle: string, reason?: string) {
  return sendEmail({
    to,
    subject: `⚠️ Pemberitahuan Takedown Karya: "${karyaTitle}"`,
    html: `
